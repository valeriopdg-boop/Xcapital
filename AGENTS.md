...
- Editorial system: articles are markdown files in `content/articles/`, compiled to `src/generated/articles.json` by `scripts/build-articles.mjs` (predev/prebuild hooks). Decap CMS admin UI lives in `public/admin/` (local: `npm run cms`; production: GitHub OAuth via `api/auth.js` + `api/callback.js`, env `GITHUB_CLIENT_ID`/`GITHUB_CLIENT_SECRET`). Article pages render at `/insight/<slug>/` with a LinkedIn share button (phase 1; automatic posting via LinkedIn Posts API is phase 2). Newsletter subscription is pending a provider account (Brevo suggested).
